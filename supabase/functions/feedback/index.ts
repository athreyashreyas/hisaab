/**
 * The relay behind Settings -> Make Hisaab Yours.
 *
 ...
 *     back to the person who registered with Hisaab, with no address to copy
 *     out by hand and no admin screen to build.
 *
 * Note this is the one thing in Hisaab that crosses the encryption boundary by
 * design: a message is written to be read by a person, so it travels as text.
 * The app never puts ledger data in it.
 *
 ...
 */
const FROM = Deno.env.get('FEEDBACK_FROM') ?? 'Hisaab <onboarding@resend.dev>';
...
Deno.serve(async (req: Request) => {
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      reply_to: replyTo,
      subject: subject || `Hisaab · ${kind === 'idea' ? 'Feature idea' : 'Bug report'}`,
      text: body,
    }),
  });
