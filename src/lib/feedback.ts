/**
 * Writing to the person who makes Hisaab.
 *
 ...
 *
 * Nothing here touches the vault. A message is the sender's own words, typed to
 * be read by somebody, so it travels as plain text while the ledger it was
 * written about stays sealed. The footer carries a version and a device and no
 * figure from the ledger at all.
 *
...
export function composeFeedback(
  ...
  return {
    subject: `Hisaab ${ctx.version} · ${FEEDBACK_KINDS[kind].subject}`,
    body: `${message.trim()}\n\n---\n${footer.join('\n')}\n`,
  };
}
